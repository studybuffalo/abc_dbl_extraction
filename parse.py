...
def price_data(url, din, brand_name_temp, generic_name, unit_price,
			   lca_temp, unit_issue):
	"""Process received strings into a price_data list.
	
	  Takes the html strings provided and further processes them 
	  before returning the completed list.
		
	  Args:
		url: url containing the supplied data
		din: html string containing the product DIN
		brand_name_temp: html string containing: brand name +/- 
						 strength +/- route +/- dosage form
		generic_name: html string containg product generic name
		unit_price: html string containing product unit price
		lca_temp: html string containg product lca +/- lca text
		unit_issue: html string containing product unit of issue
	...
	# Brand Name
	brand_name_temp = split_brand_name(brand_name_temp)
	brand_name = brand_name_temp[0]
	strength = brand_name_temp[1]
	route = brand_name_temp[2]
	dosage_form = brand_name_temp[3]
	
	# Generic Name
	generic_name = parse_generic(generic_name)
	
	# Unit Price
	unit_price = parse_unit_price(unit_price)
	...
	return [url, din, brand_name, strength, route, dosage_form,
			generic_name, unit_price, lca, lca_text, unit_issue]
			
def coverage_data(url, coverage, clients_temp, coverage_criteria,
				  coverage_criteria_sa, coverage_criteria_p):
	"""Process received strings into a coverage_data list.
	  ...
	  Args:
		url: url containing the supplied data
		coverage: html string containg the product coverage details
		clients_temp: html string containing the covered clients
		coverage_criteria: integer boolean describing if there is 
						   coverage criteria
		coverage_criteria_sa: string special authorization link
		coverage_criteria_p: string palliative care coverage link
	...
	client_list = ["(Group 1)", "(Group 66)", "(Group 66A",
				   "Income Support", "(AISH)",
				   "(Group 19824", "(Group 20400", "(Group 20403",
				   "(Group 20514", "(Group 22128", "(Group 23609"]
	clients = []
	...
	return [url, coverage, coverage_criteria, coverage_criteria_sa,
			coverage_criteria_p, clients[0], clients[1], clients[2],
			clients[3], clients[4], clients[5], clients[6],
			clients[7], clients[8], clients[9], clients[10]]

def special_auth_data(url, special_auth, special_auth_link):
	"""Process received stirngs into a special_auth_data list.
	  ...
	  Args:
		url: url containing the supplied data
		special_auth: html string containing titles of special 
					  authorization forms
		special_auth_link: html string containing linkes to special
						   authorization forms
...
def ptc_data(url, ptc_temp):
	"""Process received stirngs into a ptc_data list.
	  ...
	  Args:
		url: url containing the supplied data
		ptc_temp: html string containing PTC codes and titles
	...
	# PTC
	ptc_temp = parse_ptc(ptc_temp)
	ptc_num_1 = ptc_temp[0]
	ptc_text_1 = ptc_temp[1]
	ptc_num_2 = ptc_temp[2]
	ptc_text_2 = ptc_temp[3]
	ptc_num_3 = ptc_temp[4]
	ptc_text_3 = ptc_temp[5]
	ptc_num_4 = ptc_temp[6]
	ptc_text_4 = ptc_temp[7]
	
	return [url, ptc_num_1, ptc_text_1, ptc_num_2, ptc_text_2,
			ptc_num_3, ptc_text_3, ptc_num_4, ptc_text_4]

def atc_data(url, atc):
	"""Process received stirngs into a atc_data list.
	  ...
	  Args:
		url: url containing the supplied data
		atc: html string containing ATC codes
	...
	# URL
	url = parse_url(url)
	temp_atc = parse_atc(atc);
	return [url, temp_atc[0], temp_atc[1], temp_atc[2], temp_atc[3],
			temp_atc[4], temp_atc[5], temp_atc[6], temp_atc[7], 
			temp_atc[8], temp_atc[9]]

def extra_data(url, date_listed, date_discontinued, manufacturer,
			   schedule, interchangeable):
	"""Process received stirngs into a extra_data list.
	  ...
	  Args:
		url: url containing the supplied data
		date_listed: html string containing date product was listed
					 on the DBL
		date_discontinued: html string containing date product was 
						   removed from the DBL
		manufacturer: html string containing the product manufactuer
		schedule: html string containing the drug schedule
		interchangeable: integer boolean indicating if product has 
						 interchangeable products on DBL
	...
	# Date Listed
	date_listed = parse_date(date_listed)
	
	# Date Discontinued
	date_discontinued = parse_date(date_discontinued)
	
	# Manufacturer
	manufacturer = parse_manufactuer(manufacturer)
	
	return [url, date_listed, date_discontinued, manufacturer, schedule, interchangeable]



def split_brand_strength(text):
	'''Splits text into brand and strength at the first number.'''
	
	search = re.search(r"\s\d.+$", text)
	
	if search != None:
		index = search.start()
		output = [text[:index].strip(), text[index:].strip()]
	else:
		output = [text, ""]
...
def parse_url(text):
	'''Extracts the 10 digit URL ID.'''
	
	text = re.search(r"(\d{10})", text).group(1)
...
def parse_date(text):
	'''Converts dates to MySQL standard (YYYY-MM-DD).'''
...
def parse_lca(text):
	...
	if index == -1:
		...
		if "N/A" in text:
			...
			output = [text[0:index].strip(),
					  text[index + 1:-1].strip()]
