...
def organize_save_files(url, html, price, coverage, special, ptc, atc, extra):
    ...
    save_files = SaveFiles(url, html, cPrice, cCoverage, cSpecial, cPTC,
                          cATC, cExtra)
...
def save_data(content, save):
    ...
    try:
        ...
        log.warning('Unable to write %s to url.txt: %s', content.url, error)

    # Save the price data
    price = [content.url, content.din.parse, content.bsrf.brand,
             content.bsrf.strength, content.bsrf.route, content.bsrf.form,
             content.generic_name.parse, content.unit_price.parse,
             content.lca.value, content.lca.text, content.unit_issue.parse]

    try:
        save.price.writerow(price)
    except Exception as error:
        log.warning('Unable to write %s to price.csv: %s', content.url, error)
    ...
    try:
        ...
        log.warning(
            'Unable to write %s to coverage.csv: %s', content.url, error
        )

    # Save the special authorization data
    special = []

    for item in content.special_auth:
    ...
    try:
        ...
        log.warning(
            'Unable to write %s to special.csv: %s', content.url, error
        )
    ...
    try:
        ...
        log.warning('Unable to write %s to ptc.csv: %s', content.url, error)
    ...
    try:
        ...
        log.warning('Unable to write %s to atc.csv: %s', content.url, error)

    # Save the extra information data
    extra = [content.url, content.date_listed.parse,
             content.date_discontinued.parse, content.manufacturer.parse,
             content.schedule.parse, content.interchangeable.parse]

    try:
        save.extra.writerow(extra)
    except Exception as error:
        log.warning('Unable to write %s to extra.csv: %s', content.url, error)
    ...
    try:
        ...
        log.critical('Unable to save HTML for %s: %s', content.url, error)
